...
void test2()
{
    ...
    MysqlConn conn;
    conn.connect("root", "123456", "test", "172.17.0.1");
    steady_clock::time_point begin = steady_clock::now();
    std::thread t1(op1, 0, 2500);
    std::thread t2(op1, 2500, 5000);
    ...
    ConnectionPool* pool = ConnectionPool::getConnectionPool();
    steady_clock::time_point begin = steady_clock::now();
    std::thread t1(op2, pool, 0, 1250);
    std::thread t2(op2, pool, 1250, 2500);
    std::thread t3(op2, pool, 2500, 3750);
    std::thread t4(op2, pool, 3750, 5000);
